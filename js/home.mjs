...
async function toggleBookmark(user_id, id) {
        ...
        return `
						<tr class="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
							...
							<td class="px-4 py-4 text-sm font-bold text-primary text-right">${dotfiles.score} / 100
                                <button class="bookmark-btn text-slate-400 hover:text-yellow-400 transition-colors focus:outline-none inline-flex items-center" id="bookmark-btn" data-id="${dotfiles.id}" onclick="this.classList.toggle('text-yellow-400'); this.classList.toggle('text-slate-400');">
